function GraphModel(metricsInfo) {

    var metrics = metricsInfo.metrics;
    var aggregators = metricsInfo.aggregators;
    var resolution = metricsInfo.resolution;
    var timeLength = metricsInfo.timeLength;
    var sources = metricsInfo.sources;
    ...
    this.draw = function draw(cssSelector, type, height) {
        ...
        if(metricsInfo.sources) {
            metricsList = [];
            metrics.forEach(function(metric) {

                metricsInfo.sources.forEach(function(source) {
                    metricsList.push(metric + '-' + source);
                });
            });
        }

        graph = new Graph(type, cssSelector, metricsList, maxDataPoints, resolutionMillis, {height: height});
