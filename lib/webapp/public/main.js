$(function() {
    ...
    $('#identify').click(identifySources);
});
...
function configureGraphs() {
    ...
    infoList.forEach(function(info, index) {
        ...
        if(info.type == 'graph') {
            ...
            var graphId = '#g' + index;
            var graph = new GraphModel(graphDef.metrics);
            // var graph = new GraphModel(graphDef.metrics.metrics, graphDef.metrics.aggegators, graphDef.metrics.resolution, graphDef.metrics.timeLength, graphDef.metrics.sources);
...
function parseGraphDef(line) {
    ...
    if(line) {
        ...
        if(parts.length > 4) {
            ...
            if(parts[6]) {
                metricsInfo.sources = parts[6].split(',').map(trim);
            }
...
function identifySources(metric, range) {

    var resultBox = $('#identify-sources .result');
    resultBox.hide();
    var metric = $('#identify-sources .metric').val();
    var range = $('#identify-sources .range').val();
    ...
    $.post('/identify-sources', requestData, function(sources) {
        
        var resultStr = "";
        if(sources && sources.length > 0) {
            resultStr = sources.join(', ');
        } else if(sources.error) {
            resultStr = sources.error;
        } else {
            resultStr = "No Sources Yet"
        }

        $('.syntax', resultBox).val(resultStr);
        resultBox.show();
    });
}
